...
pub fn verify_sheets_cli(args: DIFloraArgs) -> Result<()> {
...
pub fn check_excel(
    ...
    for (worksheet_name, worksheet) in workbook.worksheets().iter().skip(args.wskip) {
        for (row_index, row) in worksheet.rows().enumerate().skip(args.rskip) {
            //skip 2 -> skip headers
            crawl(
                row,
                path,
...
fn write_to_buffer_file_failure(
    file_failure: Vec<(String, Error)>,
    buffer: &mut BufWriter<Stdout>,
) -> Result<usize> {
